...
def read_ip_addresses(filename='PingScanResult.json'):
    """
    Reads a list of IP addresses from a JSON file generated by the ping scan.
...
def run_device_scan(ip_addresses):
    """
    Runs nmap OS detection scan on a list of IP addresses.
    Returns a dictionary of scan results.
    """
    scan_results = {'hosts': []}

    for ip in ip_addresses:
        print(f"[INFO] Scanning {ip} for OS information...")
...
def detailedScan():
    """
    Orchestrates the full detailed scan: read IPs, run nmap scans, save results.
    """
    ip_addresses = read_ip_addresses()
    if not ip_addresses:
        print("[ERROR] No IP addresses found to scan. Make sure PingScanResult.json exists and is valid.")
        sys.exit(1)
